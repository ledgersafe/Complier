...
var getBusiness = async function (req, res) {
    try {
        ...
        console.log("Query has completed, checking results in express");
...
var get_history = async function (req, res) {
    try {
        console.log('getting history 2')
...
var change_holder = async function (req, res) {
    try {
        ...
        if (isProposalGood) {
            ...
            var sendPromise = await channel.sendTransaction(request);
            promises.push(sendPromise); //we want the send transaction first, so that we know where to check status
                ...
                event_hub.registerTxEvent(
                    ...
                    (tx, code) => {
                        ...
                        if (code !== "VALID") {
                            ...
                            console.log(
                                "The transaction has been committed on peer " + event_hub._ep
                            );
                            resolve(return_status);
                        }
                    },
        ...
        if (results && results[1] && results[1].event_status === "VALID") {
            console.log(
                "Controller: Successfully committed the change to the ledger by the peer"
            );
...
app.use((req, res, next) => {
    res.header('Access-Control-Allow-Origin', 'http://13.82.210.187');
...
app.use('/change', function (req, res) {
    change_holder(req, res).then(function (tx_id) {
        console.log("Returned Change TX ID: \n", tx_id)
        if (tx_id) {
            res.status(200).json({ message: 'OK', tx_id: tx_id })
        } else {
            res.status(200).json({ message: 'NOK', tx_id: tx_id })
...
app.use('/getHistory', function (req, res) {
    console.log("GET HISTORY, app.use; ", req.body.assetID)
    get_history(req, res).then(function (history) {
